...
Version PhilemonTerminalView<< EM 5-1024 >>"
PROJECTNAME PhilemonTerminalView .
PROJECTCATEGORY Philemon .
PREREQUISITES PhilemonTextEmphasisSupport .
...
file: anURL context: contextURL
	"anURL can be nil or an absolute or relative path to the contextURL" 
	anURL isNil 
		ifTrue:
			[file := nil. 
			^self].
	('http' occursIn: anURL at: 1)
		ifTrue:[	file := anURL ]
		ifFalse: [ file := contextURL , '/', anURL ]!
...
requestCursorAt: point 
	...
	isTyping := true.
	"clear the old"
	(cursor notNil and:[self grid includesLocation: cursor]) "if coming from bigger screen where cursor is outside new bounds"
		ifTrue:
			[(self grid row: cursor x column: cursor y)
...
aboutToRemoveFromTerminalForm: aForm 
	...
	image isNil
		ifFalse:[aForm graphics removeGO: self image fromLayer: 1].
	aForm grid 
